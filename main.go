...
type SecureShellVulnerabilityReport struct {
	Banner            string
...
func (report *SecureShellVulnerabilityReport) IsVulnerable() bool {
	return (report.SupportsChaCha20 || report.SupportsCbcEtm) && !report.SupportsStrictKex
}

func (report *SecureShellVulnerabilityReport) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		SecureShellVulnerabilityReport
		Vulnerable bool
	}{
		*report,
		report.IsVulnerable(),
...
func exchangeBanners(connrw *bufio.ReadWriter) (string, error) {
	...
	if _, err := connrw.Write([]byte("SSH-2.0-GoScan\r\n")); err != nil {
...
func performVulnerabilityScan(address string, scanMode ScanMode) (*SecureShellVulnerabilityReport, error) {
	var conn net.Conn
	...
	report := new(SecureShellVulnerabilityReport)
...
func printReport(report *SecureShellVulnerabilityReport, outputJson bool) error {
	if !outputJson {
		...
		if report.IsVulnerable() {
			...
			fmt.Println("The scanned peer is VULNERABLE to SecureShell.")
			color.Unset()
		} else {
			color.Set(color.FgGreen)
			fmt.Println("The scanned peer supports SecureShell mitigations and can establish")
			fmt.Println("connections that are NOT VULNERABLE to SecureShell. Glad to see this.")
...
func printVersion() {
	fmt.Println("SecureShell Vulnerability Scanner " + Version)
}
...
func main() {
	...
	var report *SecureShellVulnerabilityReport
	if *connectPtr != "" {
